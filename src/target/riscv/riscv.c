...
static bool is_riscv011_version(struct target* target)
{
	riscv_info_t *info = (riscv_info_t *)target->arch_info;
	return info->dtm_version == 0;
}
...
int riscv_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	breakpoint->set = true;
	KENDRYTE_LOG_I("add breakpoint at 0x%" TARGET_PRIxADDR "", breakpoint->address);
	return ERROR_OK;
}
		...
		enum target_register_class reg_class)
{
	...
	*reg_list = calloc(*reg_list_size, sizeof(struct reg *));
	...
	for (int i = 0; i < *reg_list_size; i++) {
		...
		struct reg* regi = &target->reg_cache->reg_list[i];
		(*reg_list)[i] = regi;
	}
...
void riscv_change_debug_info(struct target* target, int mode, int hartid)
{
	struct target_type *tt = get_target_type(target);
	tt->change_debug_info(target, mode, hartid);
}
...
struct target_type riscv_target = {
	...
	.change_debug_info = riscv_change_debug_info,
...
{
	memset(r, 0, sizeof(riscv_info_t));
	r->dtm_version = 1;
...
int riscv_init_registers(struct target *target)
{
	...
	target->reg_cache = calloc(1, sizeof(struct reg_cache));
...
#undef DECLARE_CSR
	...
	qsort(csr_info, DIM(csr_info), sizeof(struct csr_info), cmp_csr_info);
	...
	info->registers_initialized = true;
	return ERROR_OK;
}
