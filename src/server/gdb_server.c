...
static int gdb_select_debug_mode_packet(struct connection *connection,
	char const *packet, int packet_size)
{
	char* end;
	int mode = strtoul(packet + 1, &end, 16);
	...
	{	// not support
		LOG_WARNING("unknonw debug mode: %d", mode);
		return ERROR_FAIL;
	}
	return ERROR_OK;
}
...
static int gdb_input_inner(struct connection *connection)
{
	...
	do {
		...
		if (packet_size > 0) {
			...
			switch (packet[0]) {
				case '#':
					retval = gdb_select_debug_mode_packet(connection, packet, packet_size);
					break;
